...
//				goto end;
	...
	public function login_wechat() {
		...
		$access_token = json_decode($this->wechat->access_token($code), TRUE);

		$wechat_user = json_decode($this->wechat->get_user($access_token), TRUE);

		// 判断用户是否存在
		$user = $this->user_model->get_user(array('oauth_wechat' => $wechat_user['openid']));
		if(!empty($user)) {
